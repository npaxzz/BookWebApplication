...
public class ItemService {
    ...
    public ItemDTO convertToDTO(Item item) {
        Set<String> categoryNames = item.getCategories().stream()
        ...
        return new ItemDTO(item.getId(), item.getTitle(), item.getCreator(), item.getDescription(),
                           typeName, categoryNames, reviewDTOs, imageUrl);
    }
    ...
    public ItemDTO createItem(Item item) {
        Set<Category> categories = item.getCategories().stream()
    ...
    public ItemDTO updateItem(Long id, Item updatedItem) {
        ...
        Set<Category> categories = updatedItem.getCategories().stream()
            .map(c -> categoryRepository.findById(c.getId())
                .orElseThrow(() -> new RuntimeException("Category not found")))
            .collect(Collectors.toSet());
